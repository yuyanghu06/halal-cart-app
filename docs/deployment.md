...
Apple requires an enrolled Apple Developer team, a primary App ID with Sign in with Apple, an associated Services ID for web, Team ID, Key ID and a Sign in with Apple `.p8` signing key. Configure web domain `wbbnwbkpzoggffmvnqkh.supabase.co` and return URL `https://wbbnwbkpzoggffmvnqkh.supabase.co/auth/v1/callback`. Put the Services ID first in Supabase's allowed client IDs for web OAuth. Generate the Apple client-secret JWT privately and renew it before its maximum six-month expiry. Apple Developer is now signed in. The personal team `557D6QNLJV` explicitly reports that its Apple Developer Program membership has expired, with renewal date August 26, 2026 and annual fee US$99. Membership renewal by the user is the current prerequisite; no renewal, payment, Apple identifiers, keys, or provider changes were performed. The personal account page is left open for handoff. Apple remains disabled; no credentials or successful Apple sign-in are claimed.
